...
namespace BaseGame.Editor
{
    ...
    {
        protected BaseGameEditor parentEditor;

        /// <summary>
        /// Constructor.
        /// </summary>
        /// <param name="editor">The parent editor.</param>
        public EditorTab(BaseGameEditor editor)
        {
            parentEditor = editor;
        }
        ...
        protected T LoadZipJsonFile<T>(string path) where T : class
        {
            if (!File.Exists(path))
            {
                return null;
            ...
            var file = new StreamReader(path);
            var fileContents = file.ReadToEnd();
            fileContents = StringToolbox.Base64ZipDecodeString(fileContents);
            var data = fsJsonParser.Parse(fileContents);
            object deserialized = null;
            var serializer = new fsSerializer();
            serializer.TryDeserialize(data, typeof(T), ref deserialized).AssertSuccessWithoutWarnings();
        ...
        protected void SaveZipJsonFile<T>(string path, T data) where T : class
        {
            fsData serializedData;
            var serializer = new fsSerializer();
            serializer.TrySerialize(data, out serializedData).AssertSuccessWithoutWarnings();
